...
static int resize(rt_dstr_t *const thiz, size_t new_size)
{
    char *p = NULL;
...
static rt_dstr_t *ahead(rt_dstr_t *thiz, const char *src, size_t n, size_t old_size)
{
...
static rt_dstr_t *behind(rt_dstr_t *thiz, const char *src, size_t n, size_t old_size)
{
...
static rt_dstr_t *xcat(rt_dstr_t *thiz, const char *src, size_t n, save_t fun)
{
    int res = 0;
    ...
    res = resize(thiz, new_size);
    if (res == -1)
    {
        dbg_log(DBG_ERROR, "resize error\n");
        goto err;
    }
...
{
    return xcat(thiz, src, strlen(src), ahead);
}
...
{
    return xcat(dst, src->str, n, behind);
}
...
{
    return xcat(dst, src->str, n, ahead);
}

static void param_process(rt_dstr_t *const d1, rt_dstr_t *const d2, char **s1, char **s2)
{
    if (d1 != NULL)
        *s1 = d1->str;
    else
...
int rt_dstr_cmp(rt_dstr_t *const dstr1, rt_dstr_t *const dstr2)
{
    ...
    param_process(dstr1, dstr2, &s1, &s2);
    return strcmp(s1, s2);
}
...
rt_dstr_t *rt_dstr_sprintf(rt_dstr_t *thiz, const char *fmt, ...)
{
    ...
    if (resize(thiz, new_len + 1) == -1) /* allocated space */
    {
        va_end(ap);
        goto err;
    }
...
rt_dstr_t *rt_dstr_append_printf(rt_dstr_t *thiz, const char *format, ...)
{
    ...
    if (resize(thiz, old_len + new_len) == -1)
    {
        va_end(ap);
        goto err;
    }
